...
class icon
{
    ...
    public function __construct($unverified_icon_id=null)
    {
        $this->description = "";
...
class Toast
{
    public function __construct($strong_text, $error_message)
    {
        $cConfigs = new \config\ConfigurationFile();
...
class Alert
{
    ...
    public function __construct($strong_text ="Unknown Error", $error_message = "An unknown error occured.", $hault_execution = false)
    {
...
class navbar
{
    ...
    public function Add_Link($href, $text_to_display, $echo = true, $class = "")
    {
        if ($echo) {
    ...
    public function Close_Navbar($echo = true)
    {
        if ($echo = true) {
            echo '</ul>
          </nav>';
...
class table
{
    ...
    private $classlist;
    private $override;
    public function __construct($id = "mt", $echo = true, $classlist = "", $override = "")
    {
        $this->table_id = $id;
    ...
    public function Close_Table($echo = true)
    {
        if ($echo) {
            echo '</table>';
            echo '</div>';
...
{
    private $classlist;
    public function __construct($echo = true, $classlist="")
    {
        $this->classlist = $classlist;
        if ($echo) {
            echo '<thead>
    ...
    public function Add_Header($text, $scope="col", $echo = true)
    {
        if ($echo) {
    ...
    public function Close_Header($echo = true)
    {
        if ($echo) {
            echo '</tr>
        </thead>';
...
{
    private $body_id;
    public function __construct($id = "", $echo = true)
    {
        $this->body_id = $id;
        if ($echo) {
    ...
    public function Close_Body($echo = true)
    {
        if ($echo) {
            echo '</tbody>';
        } else {
...
class Table_Row
{
     ...
     */
    public function __construct(int $num_of_cols, array $values, array $tr_data_context = array(), array $three_dots_context = array(), $echo = true, $tooltip = "", $td_data_context = array(), $classlist = "", $row_id = "")
    {
        $this->classlist = $classlist;
        $this->echo = $echo;
        $this->current_string = "";
...
class context_menu
{
     ...
     */
    public function __construct($id = "context_menu", $tbody_id = "modalmt")
    {
     ...
     */
    public function Add_Action(string $text_to_display, array $data_context, $Red_Color = false)
    {
    ...
    public function Close_Context_Menu()
    {
        echo '</ul></div>';
    }
}
...
class drop_down_menu
{
     ...
     * WARNING THIS CLASS DEPENDS ON A JAVASCRIPT FUNCTION CALLED Show_Element_If_True(element to hide/show,true = show[css_display=block] false = hidden[css_display=none])
     */
    public function __construct($id = "drop_down_menu", $echo = true)
    {
     ...
     * @param array an array of complex data you want to store as json in the data_context attribute
     */
    public function Add_Action(string $text_to_display, array $data_context, $echo = true)
    {
        $a_string = "";
    ...
    public function Add_Divider($echo = true)
    {
        if ($echo) {
            echo '<li class="divider list-group-item"></li>';
        } else {
    ...
    public function Close_Context_Menu($echo = true)
    {
        if ($echo) {
            echo '</div></div>';
        } else {
